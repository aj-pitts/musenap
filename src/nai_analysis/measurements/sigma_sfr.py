...
class SigmaSfrMAP(MeasurementMAP):
    ...
    def compute(self, engine: "MeasurementEngine") -> MuseMAP:
        ...
        sfr_map = MuseMAP.empty_from_binmap("SFRSD", spatial_bins)
        bm = MuseMapBitMask()
    ...
    def plot(self, engine: "MeasurementEngine", directory: str, figname: str):
        ...
        map_data.plot_map(directory=directory, figname=figname, title=r"$\mathrm{log \Sigma_{SFR}}\ \left( \mathrm{M_{\odot}\ yr^{-1}\ kpc^{-2}\ spx^{-1}} \right)$",
                          show = False, save = True, cmap='rainbow')
        map_data.plot_hist(directory=directory, figname=figname, title=r"$\mathrm{log \Sigma_{SFR}}\ \left( \mathrm{M_{\odot}\ yr^{-1}\ kpc^{-2}\ spx^{-1}} \right)$",
